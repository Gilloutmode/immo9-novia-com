...
4. **Go 1** : « Go » d'une personne de `state/approvers.json`, en réponse à la carte. Je l'enregistre avec `outbox_approve.py` (auteur, horodatage, texte). Pour une pièce sans production payante ni rendu long, le Go 1 est facultatif : je présente directement le package final.
5. **Go 2, publication** : « Go publie » ou « publie », même règle d'auteur et de réponse. Je publie avec `skills/novia-publish`, qui refuse toute pièce sans enregistrement d'approbation. Puis je consigne dans `learning/CONTENT_LEDGER.md`.
