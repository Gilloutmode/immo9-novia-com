...
skills-shared/        8 skills réutilisables installés pour tous les agents, et la liste des skills à prendre à leur source publique
...
docs/                 installation pas à pas, architecture, matrice des 39 fonctionnalités, feuille de route, sécurité, mémoire, état de l'art, limites, prompt Claude Code
```
...
3. `bash scripts/install.sh` (simulation) puis `bash scripts/install.sh --apply` : déclare l'agent, le compte Telegram, fusionne la configuration, installe les skills partagés.
4. Déclarer les variables d'environnement (`config/env.example`) dans l'environnement du gateway, redémarrer le gateway.
...
La correspondance ligne par ligne avec les 39 fonctionnalités demandées : `docs/MATRICE-FONCTIONNALITES.md`.
