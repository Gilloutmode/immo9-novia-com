...
- `sources-veille.json` : 22 sources autorisées (officiel, professionnel, presse nationale et locale), dont 11 flux RSS vérifiés par HTTP à la livraison, et le dispositif d'alertes Google en RSS pour les mentions de marque.
...
Novia Com n'est pas né en une séance. C'est la transposition, au métier d'IMMO9 et au cadre OpenClaw, d'un studio de contenu que Gil Benittah fait tourner en production depuis plusieurs mois sur son propre agent Hermes : charte et doctrine séparées, contrat machine, deux validations par pièce, gates de qualité, budget annoncé avant toute dépense, boucle d'apprentissage hebdomadaire, tâches planifiées silencieuses, apprentissage du goût de l'opérateur. Ces méthodes ont été éprouvées, corrigées et re-corrigées sur des pièces réelles publiées, avec leurs échecs (un studio qui exigeait six validations par pièce, une voix qui n'était pas celle de l'opérateur, des crons qui parlaient pour ne rien dire), avant d'être transmises et améliorées ici.

Le travail consacré à Novia Com représente **environ 234 heures cumulées**, réparties de mai à septembre 2026 :

| Phase | Période | Ce qui a été fait |
|---|---|---|
| **Cadrage avec IMMO9** | Mai à juin | Rythme de collaboration et snapshot hebdomadaire, liste des 32 fonctionnalités et de leurs niveaux d'autonomie, appels de suivi, diagnostic en lecture seule de l'infrastructure existante (agents par ville, daemon mémoire, isolation des portefeuilles), décisions d'architecture : agent isolé, mémoire, connecteurs |
| **Transposition à OpenClaw** | Plus d'un mois, en continu | Rendre compatible avec OpenClaw ce qui tournait sur Hermes, pour que Novia Com fonctionne aussi bien que l'agent d'origine : lecture de la documentation et vérification de chaque commande sur la version cible, schéma de configuration extrait de la CLI (comptes et groupes Telegram, références de secrets), skills relus et nettoyés pour Linux (34 audités, 6 retenus, 7 référencés), verrous portés par des scripts, crons idempotents, installateur rejouable avec sauvegardes |
| **Recherche** | Août à septembre | Huit recherches multi-moteurs (Google, Brave, Exa, Linkup, Perplexity, Firecrawl, pouls social sur trente jours), plus de soixante sources uniques par requête, dépouillées source par source par deux agents indépendants : état de l'art des agents IA en communication immobilière, connecteurs et tarifs relus en direct, analytics et social listening, pratiques OpenClaw, dispositifs immobiliers 2026, salons, sources de veille. Ce qui n'était pas sourcé au niveau officiel a été marqué comme tel ; les chiffres marketing invérifiables ont été écartés et listés |
| **Construction** | Août à septembre | Charte, doctrine, base de connaissance avec niveaux de confiance, 13 skills et leurs scripts, gabarits, fragment de configuration, manifeste des crons, scripts d'installation, de diagnostic, d'acceptation et de mise à jour, documentation, prompt d'installation guidée |
| **Vérification** | Septembre | 84 tests hors ligne sur une copie complète du dépôt (hygiène, configuration Telegram et crons avec OpenClaw simulé, verrou d'onboarding, quinze refus d'approbation, publication avec réponses illisibles, refus serveur, issue inconnue, reprise sans doublon, réparation des traces, contrôles éditoriaux, production) ; installation complète exécutée sur un profil OpenClaw jetable ; rendus contrôlés visuellement ; onze flux RSS validés par HTTP à la livraison |
| **Relectures à froid** | Septembre | Cinq passes adversariales d'un second modèle (GPT-6 Astra) sur le dépôt entier, avec reproductions ciblées. Scores : 48, 68, 76, 82, 84 sur 100. Chaque passe a produit des défauts réels, tous corrigés et couverts par un test : groupes Telegram déclarés au mauvais endroit, crons découpés par un retour à la ligne, approbation non liée au package puis au message, faux succès sur une réponse asynchrone, resoumission après un délai d'attente, verrou acquis trop tard, fichiers locaux versionnés, et une dizaine de points de second ordre. Le point de fond restant, l'isolation système, est documenté comme limite de la version 1 plutôt que masqué |
| **Livraison** | Septembre | Dépôt privé, historique de commits explicite, journal des versions, prompt d'installation guidée, mail de démarche avec ce prompt intégré, maintenance hebdomadaire |
