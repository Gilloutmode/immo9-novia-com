...
- Testé sur OpenClaw 2026.9.4. Les versions 2026.6.x ne sont pas supportées par l'installateur (`config patch --replace-path`, `cron.skipMissedJobs`).
...
## Côté organisation
- L'agent ne remplace pas la validation : deux « Go » par pièce sont attendus, chacun comme message seul (« Go », « Go publie ») en réponse à la carte ; une phrase qui ajoute une condition ou une précision est refusée, les précisions se donnent dans un message à part. Sans réponse, les pièces expirent au bout de sept jours, sans relance.
