...
Étape 4 · Installation : montre-moi la sortie de bash scripts/install.sh (simulation). Explique chaque action prévue, y compris les modifications hors de l'agent (skills managés, cron.skipMissedJobs, skills.entries.novia-*). Après mon ok, exécute bash scripts/install.sh --apply. Vérifie ensuite openclaw agents list --bindings, openclaw config validate et openclaw skills check --agent novia-com, puis openclaw daemon restart et openclaw doctor.
